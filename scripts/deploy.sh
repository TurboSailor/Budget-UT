...
pkg="$("${ADB[@]}" shell "click list 2>/dev/null" | tr -d '\r' | sed -n 's/^budget\.turbosailor\t.*/&/p')"
if [ -z "$pkg" ]; then
    echo "!! budget.turbosailor is not in click list; leftovers:"
    sudo_sh "ls -d /opt/click.ubuntu.com/budget.turbosailor 2>/dev/null"
    die "install failed (clean up with: sudo rm -rf /opt/click.ubuntu.com/budget.turbosailor)"
fi
echo ">> click list: $pkg"

prof="$(sudo_sh "aa-status" | sed -n '/budget\.turbosailor/p' | head -n3)"
if [ -z "$prof" ]; then
    echo ">> apparmor profile missing, regenerating hooks"
    sudo_sh "aa-clickhook -f"
    prof="$(sudo_sh "aa-status" | sed -n '/budget\.turbosailor/p' | head -n3)"
fi
[ -n "$prof" ] || die "apparmor profile for budget.turbosailor was not generated"
echo ">> aa-status:"
echo "$prof"

ver="$(printf '%s' "$pkg" | awk '{print $2}')"
echo ">> app id: budget.turbosailor_budget_$ver  (tap the icon; adb launches lack a trust session)"
